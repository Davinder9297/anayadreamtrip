...
const TestimonialSlider = ({ serviceData, appointmentReviews }) => {
    ...
    const testimonials = serviceData?.reviews || appointmentReviews || defaultTestimonials;
    ...
    useEffect(() => {
        ...
        items.forEach((item, index) => {
            item.style.transform = `translate(${offsetX * index}px, ${-offsetY * index}px) scale(${1 - scaleFactor * index})`;
        ...
        const moveToNext = () => {
            ...
            firstItem.style.transform = `translate(${offsetX * items.length}px, ${offsetY * items.length}px) scale(0.8)`;
        ...
        >
            <div className='py-12 px-20 flex justify-between'>
                        <div sm={6} className='w-[40%]'>
                            <h5 className='text-secondary mt-10 text-xl font-jost font-bold'>TESTIMONIALS</h5>
                            <div className='text-secondary mt-5 leading-snug text-5xl font-bold font-jost'>
                                What People Say<br/> About Us
                            </div>
                        </div>
                        <div lg={6} className='flex justify-center pt-5 pb-12 xsm:pt-10 sm:pt-16 w-[60%]'>
                                ...
                                {testimonials.map((testimonial, index) => (
                                    ...
                                    >
                                        <div className="flex items-center space-x-4 xsm:space-x-1 xsm:mt-6 sm:mt-4 md:mt-2">
                                            ...
                                            />
                                            <div>
                                                <h3 className="font-bold text-lg xsm:text-[12px]">{testimonial.name}</h3>
